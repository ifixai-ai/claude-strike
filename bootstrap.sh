...
if [[ -n "${CLAUDE_STRIKE_SKIP_SPEC_KIT:-}" ]]; then
  ...
  echo "running: specify init --here --ai claude --force  (spec-kit @ $SPEC_KIT_REF)"
  if ! uvx --from "git+https://github.com/github/spec-kit.git@$SPEC_KIT_REF" specify init --here --ai claude --force; then
